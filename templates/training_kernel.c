...
void em_train(float *input_data, 
              float *component_memberships, 
              float *loglikelihoods,
              int num_components, int num_dimensions,int num_events,
              int min_iters,int max_iters, 
              float *ret_likelihood) {
    

    float* data_by_dimension
    components_t components;

    data_by_dimension  = (float*)malloc(sizeof(float) *
                              num_events * num_dimensions);
    
    for(int e = 0; e < num_events; e++) {
        for(int d = 0; d < num_dimensions; d++) {
            data_by_dimension[d * num_events + e] =
                input_data[e * num_dimensions + d];
        }
    }
    ...
    while(iters < min_iters || change > epsilon && iters < max_iters) {
        ...
        estep1(data_by_dimension,&components, component_memberships,num_dimensions,num_components,num_events,loglikelihoods);
        ...
        mstep_covar(data_by_dimension,&components,component_memberships,num_dimensions,num_components,num_events);
...
void seed_covars(components_t* components, 
                  float* data_by_event,
                  float* means,
                  int num_dimensions,
                  int num_events,
      ...
      for(int j=0; j < num_events; j++) {
        if(row==col) {
          components->R[row*num_dimensions+col] +=
              (data_by_event[j*num_dimensions + row])*(data_by_event[j*num_dimensions + row]);
        }
      }
      if(row==col) {
        components->R[row*num_dimensions+col] /=
...
void seed_components(float *data_by_event, 
                      components_t* components,
                      int num_dimensions,
                      int num_components,
                      int num_events) {
    ...
    average_variance(data_by_event,
                      means,
                      num_dimensions,
                      num_events,
                      &avgvar);    
...
void average_variance(float* data_by_event,
                      float* means,
                      int num_dimensions, 
                      int num_events,
                      float* avgvar) {
...
void compute_average_variance (float* data_by_event,
                              components_t* components,
                              int num_dimensions,
                              int num_components,
                              int num_events) {
...
float estep2_events (components_t* components,
                    float* component_memberships,
                    int M, int n, int N) {
